...
module attributes { transform.with_named_sequence } {
  transform.named_sequence @__attention_main(%variant_op: !transform.any_op {transform.consumed}) {
    ...
    transform.apply_registered_pass "iree-llvmgpu-prefetch-shared-memory" to %func_11 : (!transform.any_op) -> (!transform.any_op)
  ...
  transform.named_sequence @custom_attention(%attention: !transform.any_op {transform.readonly}) {
    ...
    %attn = transform.param.constant #iree_codegen.translation_info<TransformDialectCodegen
                                                                               codegen_spec = @__attention_main> -> !transform.any_param
